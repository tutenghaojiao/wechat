<?php
/**
 *description         全局返回码（错误码）翻译
 *FILE_NAME          wechat
 *author             周天君 ztj1030@qq.com
 *date               2018/4/1 20:33:18
 */
...
//—————————————————————练习部分2018年4月2日16:17:59————————————————————————
	...
	//正则匹配（把错误码复制到在线工具进行操作）
	//（\d）\s (开头匹配规则:数字夹空白) $1=>'(替换结果)
	//(.*?)(\n)(结尾匹配结果：任意字符加换行)		$1',$2(替换结果)
	/**
	 * 1.声明错误码属性（把所有的错误码都写入属性中）
	 */
	protected $error = [
		-1=>'系统繁忙，此时请开发者稍候再试',
		0=>'请求成功',
	...
	{
		//p ($data);die();//需要错误判定的数据
		if(!isset($data['errcode'])){
			return $data;
		}
		//data里面有错误码
		//在error属性中能找见该错误码，则把对应的中文提示返回
